import config from "../apps.json";
import {calcite, heroicon, iconSvg, register} from "./icons.js";

import arrowUpRight from "heroicons/24/outline/arrow-up-right.svg?raw";
import bellAlert from "heroicons/24/outline/bell-alert.svg?raw";
import chartBarSquare from "heroicons/24/outline/chart-bar-square.svg?raw";
import globeEuropeAfrica from "heroicons/24/outline/globe-europe-africa.svg?raw";
import map from "heroicons/24/outline/map.svg?raw";
import square3Stack3d from "heroicons/24/outline/square-3-stack-3d.svg?raw";

import satellite3 from "@esri/calcite-ui-icons/icons/satellite-3-32.svg?raw";

// The icons an app can name in apps.json, plus the card's hover arrow. They
// live here rather than in ./icons.js so they ship in this chunk, which only
// the home page loads.
register({
  "heroicon:arrow-up-right": heroicon(arrowUpRight),
  "heroicon:bell-alert": heroicon(bellAlert),
  "heroicon:chart-bar-square": heroicon(chartBarSquare),
  "heroicon:globe-europe-africa": heroicon(globeEuropeAfrica),
  "heroicon:map": heroicon(map),
  "heroicon:square-3-stack-3d": heroicon(square3Stack3d),
  "calcite:satellite-3": calcite(satellite3),
});

// Full literal class strings so Tailwind's scanner picks them up. Each accent
// colors both the icon stroke and its tinted chip background. The ramps
// themselves are remapped to the GEO palette in src/style.css.
const ACCENTS = {
  blue: "text-blue-600 dark:text-sky-400 bg-blue-50 dark:bg-blue-500/15 group-hover:bg-blue-100 dark:group-hover:bg-blue-500/25",
  cyan: "text-cyan-600 dark:text-cyan-300 bg-cyan-50 dark:bg-cyan-500/15 group-hover:bg-cyan-100 dark:group-hover:bg-cyan-500/25",
  indigo: "text-indigo-700 dark:text-indigo-300 bg-indigo-50 dark:bg-indigo-500/15 group-hover:bg-indigo-100 dark:group-hover:bg-indigo-500/25",
  sky: "text-sky-600 dark:text-sky-300 bg-sky-50 dark:bg-sky-500/15 group-hover:bg-sky-100 dark:group-hover:bg-sky-500/25",
  teal: "text-teal-600 dark:text-teal-300 bg-teal-50 dark:bg-teal-500/15 group-hover:bg-teal-100 dark:group-hover:bg-teal-500/25",
  emerald: "text-emerald-600 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-500/15 group-hover:bg-emerald-100 dark:group-hover:bg-emerald-500/25",
  orange: "text-orange-500 dark:text-orange-400 bg-orange-50 dark:bg-orange-500/15 group-hover:bg-orange-100 dark:group-hover:bg-orange-500/25",
  amber: "text-amber-600 dark:text-amber-300 bg-amber-50 dark:bg-amber-500/15 group-hover:bg-amber-100 dark:group-hover:bg-amber-500/25",
  violet: "text-violet-700 dark:text-violet-300 bg-violet-50 dark:bg-violet-500/15 group-hover:bg-violet-100 dark:group-hover:bg-violet-500/25",
};

const FALLBACK_ACCENT = "blue";

// Each app deploys itself to the path it declares, so an app's apps.json path
// (/rfs, /previews/rfs-v3) is already its URL. The distribution resolves that
// directory URL to the app's index.
const appHref = (path) => `/${String(path ?? "").replace(/^\/+/, "")}`;

function tagPill(text) {
  return `<span class="text-xs font-medium px-2.5 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700">${text}</span>`;
}

// One app card — the original card markup, with href/icon/accent/name/
// description/tags pulled from config.
function appCard(app) {
  const accent = ACCENTS[app.accent] ?? ACCENTS[FALLBACK_ACCENT];
  const tags = (app.tags ?? []).map(tagPill).join("");
  return `
      <a href="${appHref(app.path)}" class="glass-card p-8 rounded-2xl flex flex-col h-full group relative overflow-hidden">
        <div class="absolute top-0 right-0 p-6 opacity-0 group-hover:opacity-100 transition-opacity">
          ${iconSvg("heroicon:arrow-up-right", {size: 24, className: "w-6 h-6 text-blue-600"})}
        </div>
        <div class="mb-6 p-3 rounded-xl w-fit transition-colors ${accent}">
          ${iconSvg(app.icon, {size: 32})}
        </div>
        <h3 class="text-2xl font-bold text-slate-800 dark:text-white mb-3 group-hover:text-blue-600 dark:group-hover:text-sky-400 transition-colors">${app.name}</h3>
        <p class="text-slate-600 dark:text-slate-400 leading-relaxed mb-8 grow">${app.description}</p>
        <div class="flex flex-wrap gap-2 mt-auto">${tags}</div>
      </a>`;
}

// One group: an optional heading, then a grid of its app cards. A blank
// heading renders a plain grid (no <h2>), matching the old single-grid look.
function groupSection(group) {
  const heading = group.heading
    ? `<h2 class="text-2xl font-bold text-slate-800 dark:text-white mb-6">${group.heading}</h2>`
    : "";
  const cards = (group.apps ?? []).map(appCard).join("");
  return `
    <section class="mb-10 last:mb-0">
      ${heading}
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">${cards}
      </div>
    </section>`;
}

// A group belongs to the page named by its "page" field; the home page is the
// default, so only groups moved onto a secondary page (the previews) say so.
const groupsFor = (page) =>
  (config.groups ?? []).filter((group) => (group.page ?? "home") === page);

export function renderAppLibrary(mount = document.getElementById("appLibrary")) {
  if (!mount) return; // no-op on pages without the grid (profile / terms)
  mount.innerHTML = groupsFor(mount.dataset.page || "home").map(groupSection).join("");
}
